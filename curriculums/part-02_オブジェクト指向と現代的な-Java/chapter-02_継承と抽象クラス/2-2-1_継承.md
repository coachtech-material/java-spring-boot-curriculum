...
📝 ここで初めて出てきた `instanceof` は、左のオブジェクトが指定した型かどうかを判定する真偽式です（`o instanceof Tag` は「`o` は `Tag` 型か」）。続く `(Tag) o` は、`Object` 型で受け取った `o` を `Tag` 型として扱い直すキャストで、直前の `instanceof` で型を確かめてあるので安全に行えます。
...
## 🏃 手を動かす（任意）: equals / hashCode の契約を体感する

ここは任意です（飛ばしても本筋に支障はありません）。1-1-4 の手順で JShell を起動して試します。まず `equals` / `hashCode` を実装していない `Tag` で、同じ中身を `HashSet` に入れます。次のブロックを貼り付けます。

```java
import java.util.*;
class Tag {
    String name;
    Tag(String n) { this.name = n; }
}
...
中身が同じ `"java"` を 2 つ入れたのに、要素数は `2`。重複が消えていません。次に `equals` / `hashCode` を実装した `Tag` で試し直します。まず `/reset` と入力して状態をリセットし、続けて次のブロックを貼り付けます。
...
今度は要素数が `1`。`equals` / `hashCode` をセットで実装したことで、`HashSet` が「中身が同じ `Tag`」を重複と判定できました。これが導入で触れた「半日溶かす罠」の正体です。

> 💡 `/reset` は、それまでに定義した変数やクラスをすべて消して、起動直後の状態に戻す JShell のコマンドです。同じ名前のクラスを定義し直すときに便利です。
