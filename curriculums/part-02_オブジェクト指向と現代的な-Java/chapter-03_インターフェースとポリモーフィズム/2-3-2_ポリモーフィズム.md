...
## 🏃 手を動かす（任意）: 実装の差し替えを体感する

ここは任意です（飛ばしても本筋に支障はありません）。1-1-4 の手順で JShell を起動して試します。まず契約（インターフェース）と 2 つの実装を定義します。次のブロックを貼り付けます。
...
class Email implements NotificationSender {
    public void send(String m) { System.out.println("メール送信: " + m); }
}
class Slack implements NotificationSender {
    public void send(String m) { System.out.println("Slack送信: " + m); }
}
```

同じ `NotificationSender` 型の変数に実体を入れ替え、同じ呼び出しをします（`jshell>` が入力、その下が出力）。

```text
jshell> NotificationSender sender = new Email()
sender ==> Email@1bce4f0a

jshell> sender.send("期限切れです")
メール送信: 期限切れです

jshell> sender = new Slack()
sender ==> Slack@7cd62f43

jshell> sender.send("期限切れです")
Slack送信: 期限切れです
```

`sender.send(...)` という **まったく同じ呼び出し** が、`sender` の実体に応じて違う出力になりました。これがポリモーフィズムです（`@` 以降は実行のたびに変わります）。
