...
その Bean を保持し、必要なときに取り出せるようにしている入れ物が **`ApplicationContext`** です。Spring アプリケーションが起動すると、この `ApplicationContext`（DI コンテナの実体）が立ち上がり、管理対象の Bean をすべて生成して内部に保持します。依存の注入も、Bean の取り出しも、この `ApplicationContext` を通じて行われます。そして、ここで生成される Bean は **既定でシングルトン** （コンテナ内に 1 つだけ生成し、必要とするすべてのクラスで同じインスタンスを使い回す）です。冒頭で挙げた「同じ部品があちこちで重複して作られる」という密結合の問題は、こうして解消されます。
