...
なお、本 Chapter で扱う `タスク一覧` は **読み取り専用** の画面です。新規作成・編集のフォーム（`th:action` を使った `<form>` 送信）は扱いません。データの登録・更新は REST API（`POST /api/tasks` など、3-3-1）の役割であり、画面側は一覧の表示に絞ることで、API を主役に保ちます。リンクの書き方は、一覧から各タスクの詳細へ飛ぶ程度に押さえておけば十分です。
