...
public class User {
    ...
    // --- getter / setter（IntelliJ なら Alt+Insert →「Getter and Setter」でも生成できます） ---
...
💡 **getter / setter も省略せず載せています**: コピーすればそのままコンパイルが通るよう、本セクションの 3 つのエンティティ（User / Task / Tag）は getter / setter まで含めて掲載します。後の 5-2-2 以降で `task.setTitle(...)` や `task.getTags()` のように使うため、これらが揃っていないとビルドが通りません。IntelliJ で書くなら、フィールドとコンストラクタだけ書いて Alt+Insert →「Getter and Setter」でまとめて生成しても同じものができます。
