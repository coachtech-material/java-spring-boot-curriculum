...
IntelliJ では、クラスを新規作成するときにパッケージ名を指定すれば、対応するフォルダも自動で作られます。ターミナルから先に作っておくなら次のとおりです（`taskapp/` の直下で実行します）。

```bash
# このセクションで使う 2 つのパッケージ用ディレクトリを作る
mkdir -p src/main/java/com/example/taskapp/entity
mkdir -p src/main/java/com/example/taskapp/repository
```

📝 **新しいファイルの作り方（以降の Part 5 共通）**: ここから、各 Step で新しいファイルを作っては中身を貼り付けていきます。**コードブロックの 1 行目のコメントが、そのファイルの置き場所（パス）** です。作り方は次のどちらでも構いません。

- **IntelliJ で作る**: 配置先のパッケージ（フォルダ）を右クリック →「新規」→「Java クラス」（英語 UI は New → Java Class）でクラス名（例: `User`）を入力して作成し、生成された中身を **全選択して本教材のコードで置き換え** ます。`.yml` / `.html` など Java 以外のファイルは「新規」→「ファイル」（New → File）でファイル名ごと作ります
- **ターミナルで作る**: パスを指定して `touch` で空ファイルを作り、エディタで開いて貼り付けます。例:

```bash
touch src/main/java/com/example/taskapp/entity/User.java
```

以降の各ファイルも同じ手順なので、ファイルの作り方そのものは繰り返しません（各コードブロックのパスコメントを見て、そこに作ってください）。

また、各コードブロックの **1 行目のコメント** に、操作の種類とファイルの場所を示します。これを見れば、新規作成か既存の編集か、全文か一部追記かが分かります。

- `// 新規作成: <パス>` … 新しいファイルを作り、下を全文貼り付け
- `// 全文を置き換え: <パス>` … 既存ファイルの中身を消して貼り替え（メソッド単位なら `// メソッドを置き換え: ...`）
- `// 既存に追記: <ファイル>（どこに何を）` … `import` やメソッドを指定の場所へ足す
...
```java
// 新規作成: src/main/java/com/example/taskapp/entity/User.java
...
public class User {
    ...
    // --- getter / setter（IntelliJ なら Alt+Insert →「Getter と Setter」でも生成できます） ---
...
💡 **getter / setter も省略せず載せています**: コピーすればそのままコンパイルが通るよう、本セクションの 3 つのエンティティ（User / Task / Tag）は getter / setter まで含めて掲載します。後の 5-2-2 以降で `task.setTitle(...)` や `task.getTags()` のように使うため、これらが揃っていないとビルドが通りません。IntelliJ で書くなら、フィールドとコンストラクタだけ書いて Alt+Insert →「Getter と Setter」（英語 UI は Getter and Setter）でまとめて生成しても同じものができます。
...
```java
// 新規作成: src/main/java/com/example/taskapp/entity/Task.java
package com.example.taskapp.entity;
...
public class Task {
    ...
    // --- getter / setter（IntelliJ なら Alt+Insert でも生成できます） ---
    ...
    public boolean isDone() {            // boolean の getter は isXxx()
        return done;
    }
...
```java
// 新規作成: src/main/java/com/example/taskapp/entity/Tag.java
package com.example.taskapp.entity;
...
```java
// 新規作成: src/main/java/com/example/taskapp/repository/UserRepository.java
...
```java
// 新規作成: src/main/java/com/example/taskapp/repository/TaskRepository.java
package com.example.taskapp.repository;
...
```java
// 新規作成: src/main/java/com/example/taskapp/repository/TagRepository.java
package com.example.taskapp.repository;
...
💡 このコマンドを打つと `Using a password on the command line interface can be insecure.` という警告が出ますが、ローカルでの確認用なので無視して構いません。
...
📚 **使った概念の復習**: 3-4-1（エンティティ）/ 3-4-2（リレーション・JpaRepository）/ 2-1-1（クラスとインスタンス）
