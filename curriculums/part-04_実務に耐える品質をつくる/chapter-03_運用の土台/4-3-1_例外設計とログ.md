...
次のセクションでは、ここまで見てきたアプリを「どの環境でも動かせる形」にする道具立てを学びます。開発と本番で設定を切り替える **プロファイル**、設定を外から注入する **環境変数** と優先順位、データベースのパスワードのような **シークレットの扱い**、そしてアプリを 1 つの成果物にまとめる **パッケージング** （実行可能 JAR）と **Docker イメージ化** （Buildpacks / Dockerfile）を学び、環境ごとに動かせる形にできるようになります。
