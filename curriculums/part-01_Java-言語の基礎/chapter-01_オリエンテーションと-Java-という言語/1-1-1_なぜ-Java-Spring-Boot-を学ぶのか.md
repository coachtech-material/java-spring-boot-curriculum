...
💡 とはいえ「読むだけでなく手も動かしたい」という人のために、Part 1〜2 には任意の軽量ハンズオン（JShell トラック）を用意しています。各セクション末尾の **🏃 手を動かす** で、学んだ概念をその場で実行して確かめられます。環境は Docker で 1 行（JDK のインストールは不要）、やり方は 1-1-4 にまとめてあります。読み飛ばしても本筋の理解には支障ありません。
