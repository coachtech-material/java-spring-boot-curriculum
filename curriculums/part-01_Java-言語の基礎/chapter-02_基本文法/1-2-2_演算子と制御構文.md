...
**三項演算子（条件演算子）** `条件 ? A : B` は、条件が `true` なら `A`、`false` なら `B` を返す式です。値の出し分けを 1 行で書けます。

```java
int score = 75;
String result = score >= 60 ? "合格" : "不合格";   // 条件で「値」を選ぶ
```

PHP の `$x ? $a : $b` と同じ書き方です。`if` / `else` が「処理」を分けるのに対し、三項演算子は「値」を選ぶときに使う、と捉えると使い分けられます。
...
## 🏃 手を動かす（任意）: 整数除算と switch 式を試す

ここは任意です（飛ばしても本筋に支障はありません）。1-1-4 の手順で JShell を起動して試します。`jshell>` で始まる行が入力、その下が出力です。

まず整数どうしの割り算です（式は 1 つずつ入力します）。
...
`int` どうしは小数を切り捨て、片方が小数なら結果も小数です。次の `switch` 式は複数行の 1 つのまとまりなので、ブロックごと貼り付けます。

```java
String label = switch ("admin") {
    case "admin" -> "管理者";
    case "user"  -> "一般";
    default      -> "不明";
};
...
最後に拡張 `for` です。これも 1 つのまとまりとして貼り付けます。
