...
💡 値の型が `int` ではなく `Integer` なのは、コレクションがオブジェクト（参照型）しか格納できないためです。`int` と `Integer` は自動で相互変換されます（オートボクシング）。`int`→`Integer` のように、各プリミティブ型には対応する参照型（ラッパ型）があります（`long`→`Long`、`double`→`Double`、`boolean`→`Boolean` など）。今は「コレクションの要素にはこのラッパ型を使う」と捉えておけば十分です。
...
## 🏃 手を動かす（任意）: コレクションの型安全と重複排除を試す

ここは任意です（飛ばしても本筋に支障はありません）。1-1-4 の手順で JShell を起動して試します。

まず `List` を作って 3 つ追加します。次のブロックを貼り付けます。
...
各 `add` は追加に成功すると `true` を返します。`names` を入力して中身を確かめます（`jshell>` が入力、その下が出力）。
...
`List` は順序を保ち、重複（2 つ目の `Alice`）も格納します。型に反する値を入れようとすると、コンパイルエラーで弾かれます。

```text
jshell> names.add(123)
|  Error:
...
次に `Set` です。同じ値は自動的に 1 つにまとめられます。次のブロックを貼り付けます。
...
3 つ目の `"java"` は重複なので無視され、`tags` には `java` が 1 つだけ残ります（`Set` は順序を保証しないため、表示順は環境により前後します）。確認できたら `/exit` で終了します。
