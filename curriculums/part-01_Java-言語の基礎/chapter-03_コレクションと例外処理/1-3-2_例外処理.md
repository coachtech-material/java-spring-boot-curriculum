...
まず、例外を捕まえずに起こします（`jshell>` が入力、その下が出力）。
...
これを `try` / `catch` / `finally` で受け止めます。次のブロックを貼り付けます。
...
try {
    ...
    System.out.println("finally は必ず実行");
}
```
...
`catch` で例外を受け止め、`finally` は成功・失敗にかかわらず実行されます。

> 💡 本文で学んだ **検査例外** （`IOException` など、`catch` か `throws` が必須でコンパイルエラーになるもの）は、JShell では再現できません。JShell は手早く試せるよう、トップレベルで検査例外のチェックを緩めているためです。検査例外の「コンパイル時に強制される」感覚は、Part 5 で実際の `.java` ファイルを書くときに体感できます。
