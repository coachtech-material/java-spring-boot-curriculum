...
読者はこれまでの教材を修了し、プログラミングと Web 開発の経験・概念的な土台を持っている。だが **Java の知識はゼロ** である。したがって本教材は、**Java について必要な情報を省略しない**。変数・データ型・制御構文・メソッド・配列といった基本文法も、Java ではどう書き、どう振る舞うのかを一つずつ解説する（ハンズオンを設けないだけで、解説は省かない）。
